    ...
    <View style={styles.modalContent}>
          ...
          {etapa === 1 && (
            <>
              ...
              <DropdownInput
                ...
                items={listaMedicamentos.map(m => ({ label: m.nome, value: String(m.id) }))}
            ...
            <>
              <DropdownInput
                ...
                items={Object.keys(TipoDeFrequencia)
                  ...
                  .map(k => ({ label: k.replace(/_/g, ' '), value: String(TipoDeFrequencia[k as keyof typeof TipoDeFrequencia]) }))
                }
              ...
              {Number(freqTipo) === TipoDeFrequencia.INTERVALO_HORAS && (
                <FormInput titulo="Intervalo (Horas)" value={intervalo} onChangeText={setIntervalo} keyboardType="numeric" placeHolder="Ex: 8" />
              )}
              {Number(freqTipo) === TipoDeFrequencia.HORARIOS_ESPECIFICOS && (
                <FormInput titulo="Horários" value={horarios} onChangeText={setHorarios} placeHolder="Ex: 08:00, 20:00" />
              )}
              ...
              <DropdownInput
                ...
                items={Object.keys(TipoDeAlerta)
                  .filter(k => isNaN(Number(k)))
                  .map(k => ({ label: k.replace(/_/g, ' '), value: String(TipoDeAlerta[k as keyof typeof TipoDeAlerta]) }))
                }
      ...
      <View style={{ paddingTop: 10 }}>
        {etapa < 3 ? (
          ...
          <>
            {isLoading ? <ActivityIndicator size="large" color="#0000ff" /> : <ActionButton titulo="SALVAR" onPress={handleSalvar} />}
